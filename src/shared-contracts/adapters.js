...
export function canonicalEventFromEngineEvent(event = {}, context = {}) {
  return createCanonicalEvent({
    ...
    parentEventId: event.parentEventId || context.parentEventId || "",
    rootEventId: event.rootEventId || context.rootEventId || "",
    eventGroupId: event.eventGroupId || context.eventGroupId || "",
    syncRevision: event.syncRevision || context.syncRevision || context.revision || 0,
    eventVersion: event.eventVersion || "canonical-event-1",
    ...
    rulesEngineVersion: context.rulesEngineVersion || DEFAULT_RULES_ENGINE_VERSION,
    informationConfidence: event.informationConfidence || "engine-verified",
    executionConfidence: event.executionConfidence || "engine-validated",
    importance: event.importance || "normal",
    tags: event.tags || [],
    ruleReferences: event.ruleReferences || [],
    ruleAmendmentReferences: event.ruleAmendmentReferences || [],
    undoReferences: event.undoReferences || {},
    eventKnowledgeMetadata: event.eventKnowledgeMetadata || {},
  });
}
