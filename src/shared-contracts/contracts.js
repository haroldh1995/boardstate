...
export function createCanonicalEvent(input = {}) {
  return {
    ...
    rootEventId: String(input.rootEventId || input.eventId || input.id || ""),
    eventGroupId: String(input.eventGroupId || input.groupId || ""),
    syncRevision: Number(input.syncRevision || input.revision || 0),
    eventVersion: String(input.eventVersion || "canonical-event-1"),
    ...
    informationConfidence: String(input.informationConfidence || "engine-verified"),
    executionConfidence: String(input.executionConfidence || "engine-validated"),
    importance: String(input.importance || "normal"),
    tags: Array.isArray(input.tags) ? [...new Set(input.tags.map((tag) => String(tag || "").toLowerCase()).filter(Boolean))] : [],
    ruleReferences: Array.isArray(input.ruleReferences) ? input.ruleReferences.map(String).filter(Boolean) : [],
    ruleAmendmentReferences: Array.isArray(input.ruleAmendmentReferences) ? input.ruleAmendmentReferences.map(String).filter(Boolean) : [],
    undoReferences: clonePlain(input.undoReferences || {}),
    eventKnowledgeMetadata: clonePlain(input.eventKnowledgeMetadata || {}),
  };
}
