...
export function buildLocalSave(profile, options = {}) {
  ...
  return {
    ...
    metadata: {
      ...
      aiGameplay: {
        version: activeSession.aiGameplay?.version || "",
        informationMode: activeSession.aiGameplay?.informationMode || "public-information",
        activeProfileCount: (activeSession.aiGameplay?.activeProfileIds || []).length,
        hasThreatAnalysis: Boolean(activeSession.aiGameplay?.threatAnalysis),
        hasBoardAnalysis: Boolean(activeSession.aiGameplay?.boardAnalysis),
        hasReplayAnalysis: Boolean(activeSession.aiGameplay?.replayAnalysis),
        hasLatestDecision: Boolean(activeSession.aiGameplay?.latestDecision),
