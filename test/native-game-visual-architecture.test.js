...
test("battlefield runtime uses the Commander Action Hand instead of the former bottom toolbar", () => {
  ...
  assert.match(render, /const fanStack = projection\.zIndex/);
  assert.match(render, /const fanLiftZ = projection\.liftZ/);
  ...
  assert.match(styles, /--command-deck-scroll-steps: 0/);
  assert.match(styles, /left: calc\(50% \+ var\(--hand-live-x, 0px\)\)/);
  assert.match(styles, /transform: translateZ\(0\)/);
  assert.match(styles, /\.commander-action-hand__fan\.is-free-scrolling/);
  assert.match(styles, /data-command-deck-live-center="true"/);
