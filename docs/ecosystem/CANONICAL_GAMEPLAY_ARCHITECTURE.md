...
Do not convert active gameplay into a portrait-style vertical document. No post-loading application surface or active gameplay requires vertical page scrolling. After loading, the platform adapter requests and maintains native landscape where supported. The client never interrupts gameplay with a rotate-device dialog: on platforms that cannot programmatically lock orientation, the fixed landscape composition remains authoritative while the user turns the device. The web adapter must never rotate the full application with CSS to simulate landscape inside a portrait viewport.
