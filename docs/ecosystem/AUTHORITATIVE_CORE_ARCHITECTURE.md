...
Prompt 4 persistence/replay/save architecture is documented in `PERSISTENCE_REPLAY_ARCHITECTURE.md`. Prompt 9 Question System architecture is documented in `RULES_ASSISTANT_ARCHITECTURE.md`. Prompt 10 proactive assistance architecture is documented in `PROACTIVE_ASSISTANT_ARCHITECTURE.md`. Prompt 11 local AI gameplay analysis architecture is documented in `AI_GAMEPLAY_ARCHITECTURE.md`. Later prompts will expand Event Knowledge usage in battlefield replay UI, Rules Recovery imports, Hub/Lite/Nexus interoperability, analytics, and spectator mode.
